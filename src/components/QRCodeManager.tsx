...
export const QRCodeManager: React.FC = () => {
  ...
  // Custom Logic replaced by centralized utility
  // generateQRCodeImage is no longer needed locally as downloadQRCode handles it
  // and we use the imported downloadQRCode directly.
  ...
  const handleSaveCustomization = async (qr: QRCode, customization: QRCustomization) => {
    try {
    ...
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg p-4 md:p-6 border border-pink-100 mb-8">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            value={statusFilter}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => fetchQRCodes(1, undefined, e.target.value)}
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              <form onSubmit={handleAddQR} className="space-y-6">
                <div>
                  ...
                  <input
                    type="text"
                    value={qrForm.name}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQRForm((prev: any) => ({ ...prev, name: e.target.value }))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex space-x-2">
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQRForm((prev: any) => ({ ...prev, shortCode: e.target.value.toLowerCase() }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQRForm((prev: any) => ({ ...prev, destinationUrl: e.target.value }))}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                ...
                <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-100">
                  ...
                  <button
                    type="submit"
                    className="flex items-center space-x-2 px-6 py-2 bg-pink-500 text-white rounded-lg hover:bg-pink-600 transition-colors shadow-sm"
                  >
                    <Save size={18} />
                ...
                {editingQR?.id === qr.id ? (
                  <form onSubmit={handleUpdateQR} className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditingQR(prev => prev ? { ...prev, name: e.target.value } : null)}
                    ...
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditingQR(prev => prev ? { ...prev, shortCode: e.target.value.toLowerCase() } : null)}
                      className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent text-sm"
                      required
                    />
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditingQR(prev => prev ? { ...prev, destinationUrl: e.target.value } : null)}
                      className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent text-sm"
                      required
                    />
